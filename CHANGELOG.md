...
- **模板缓存机制**：支持本地缓存模板，加快创建速度（缓存位置：`~/.bingoctl/templates/`）
- **版本/分支选择**：支持指定模板版本或分支创建项目（`-r` 参数）
- **强制刷新**：支持 `--no-cache` 参数强制重新下载模板
- **镜像配置**：支持通过 `BINGOCTL_TEMPLATE_MIRROR` 环境变量配置 GitHub 镜像
...
- 首次使用 `bingoctl create` 时会自动从 GitHub 下载模板
- 如果网络访问 GitHub 有困难，可配置镜像：`export BINGOCTL_TEMPLATE_MIRROR=https://ghproxy.com/`
